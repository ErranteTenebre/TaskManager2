...
public class Program
{
    ...
    private static void ConfigureRequestLifetime(WebApplication app)
    {
        ...
        app.UseWhen(context =>
        {
            //Проверяем, что путь запроса равен "/api/user" и метод запроса - GET
            return context.Request.Path == "/api/user";
        }, appBuilder =>
        {
            // Здесь указываем middleware, который будет применяться только к выбранным маршрутам
            appBuilder.UseMiddleware<AuthMiddleware>();
        });

        app.UseEndpoints(endpoints =>
        {
            endpoints.MapControllers(); // Зарегистрируем контроллеры
        });
